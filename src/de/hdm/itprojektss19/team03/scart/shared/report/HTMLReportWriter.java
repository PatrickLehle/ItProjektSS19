	...
	public void process(ArticleReport r) {
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			for (int k = 0; k < row.getNumColumns(); k++) {
				...
				if (i == 0) {
					result.append("<td style=\"background:#ffd874;font-weight:bold; font-size: larger; font-family: sans-serif;\">"+ row.getColumnAt(k) + "</td>");
				}
				...
				//Hier wird die formative Trennung der einzelnen Ausgaben erkenntlich gemacht
					...
					result.append("<td height=\"50\"; style=\"border-top: 3px solid #5669b1\">"+ "<b style=\"font-family: sans-serif;\">" + row.getColumnAt(k) + "</b></td>");
				}else {
					if (i > 1) {
						result.append(
								"<td height=\"25\"; style=\"border-top: 1px solid #e6e6e6; font-family: sans-serif;\">"	+ row.getColumnAt(k) + "</td>");
					}else {
